...
use Detection\Cache\Cache_Exception;
use Detection\Cache\Cache_Invalid_Argument_Exception;
use Detection\Exception\Mobile_Detect_Exception;
use Detection\Exception\Mobile_Detect_Exception_Code;
use Psr\Simple_Cache\Cache_Interface;
use Psr\Simple_Cache\InvalidArgumentException as PsrInvalidArgumentException;
/**
...
class Mobile_Detect
{
     ...
     */
    protected static array $known_cloud_front_headers = ['HTTP_CLOUDFRONT_IS_MOBILE_VIEWER', 'HTTP_CLOUDFRONT_IS_TABLET_VIEWER', 'HTTP_CLOUDFRONT_IS_DESKTOP_VIEWER'];
    protected static string $cloud_front_ua = 'Amazon CloudFront';
    /**
     * The matching regex string. Used only for debugging.
     */
    protected string $matching_regex = '';
    /**
     * The matches extracted from the regex expression. Used only for debugging.
     */
    protected array $matches_array = [];
    /**
     * HTTP headers that trigger the 'isMobile' detection to be true.
     */
    protected static array $known_mobile_positive_headers = [
        'HTTP_ACCEPT' => ['matches' => [
            // Opera Mini
     ...
     */
    protected static array $phone_devices = [
     ...
     */
    protected static array $tablet_devices = [
     ...
     * User-Agent string.
     */
    protected static array $known_user_agent_http_headers = [
     ...
     */
    public function __construct(?Cache_Interface $cache = null, array $config = [])
    {
        ...
        if ($this->config['autoInitOfHttpHeaders']) {
            $this->auto_init_known_http_headers();
        }
    }
    /**
     ...
     */
    public function auto_init_known_http_headers(): void
    {
        ...
        $known_http_headers = array_merge(array_values(self::$known_user_agent_http_headers), array_keys(self::$known_mobile_positive_headers), array_values(self::$known_cloud_front_headers));
        ...
        $http_headers = [];
        foreach ($known_http_headers as $header_name) {
            if (isset($_SERVER[$header_name])) {
                $http_headers[$header_name] = $_SERVER[$header_name];
            }
        }
        $this->set_http_headers($http_headers);
     ...
     */
    public function set_http_headers(array $http_headers = []): void
    {
        ...
        foreach ($this->get_ua_http_headers() as $alt_header) {
            if (!empty($this->http_headers[$alt_header])) {
                $user_agent .= $this->http_headers[$alt_header] . ' ';
            }
        }
        if (!empty($user_agent)) {
        ...
        if ($this->has_http_header(self::$known_cloud_front_headers[0]) || $this->has_http_header(self::$known_cloud_front_headers[1])) {
            $this->set_user_agent(self::$cloud_front_ua);
        }
    }
    /**
    ...
    public function has_http_headers(): bool
    {
        return count($this->http_headers) > 0;
    }
    protected function has_http_header(string $name): bool
    {
        return !empty($this->http_headers[$name]);
    }
    /**
     ...
     */
    public function get_http_header(string $header): ?string
    {
        ...
        $alt_header = 'HTTP_' . $header;
        //Test both the regular and the HTTP_ prefix
        if (isset($this->http_headers[$header])) {
            return $this->http_headers[$header];
        }
        return $this->http_headers[$alt_header] ?? null;
    }
    public function get_mobile_headers(): array
    {
        return static::$known_mobile_positive_headers;
    }
    /**
    ...
    {
        return static::$known_user_agent_http_headers;
    }
    /**
     * Retrieves the HTTP CloudFront headers
     * that trigger a mobile detection.
     */
    public function get_cloud_front_http_headers(): array
    {
        return static::$known_cloud_front_headers;
    }
    /**
     ...
     */
    private function prepare_user_agent(string $user_agent): string
    {
        $user_agent = trim($user_agent);
        return substr($user_agent, 0, $this->config['maximumUserAgentLength']);
    }
    /**
     ...
     * @param string $userAgent The User-Agent string.
     */
    public function set_user_agent(string $user_agent): string
    {
        $prepared_user_agent = $this->prepare_user_agent($user_agent);
        return $this->user_agent = $prepared_user_agent;
    }
    /**
    ...
    public function has_user_agent(): bool
    {
        return is_string($this->user_agent);
    }
    public function is_user_agent_empty(): bool
    {
        return $this->has_user_agent() && $this->user_agent === '';
    }
    public function get_matching_regex(): ?string
    {
        return $this->matching_regex;
    }
    public function get_matches_array(): ?array
    {
     ...
     */
    public static function get_phone_devices(): array
    {
        return static::$phone_devices;
    }
    /**
     ...
     */
    public static function get_tablet_devices(): array
    {
        return static::$tablet_devices;
    }
    /**
    ...
    public function get_rules(): array
    {
        static $rules;
        if (!$rules) {
            $rules = array_merge(static::$browsers, static::$operating_systems, static::$phone_devices, static::$tablet_devices);
        }
        return $rules;
    }
    /**
    ...
    {
        return static::$operating_systems;
    }
    /**
     ...
     */
    public function check_http_headers_for_mobile(): bool
    {
        foreach ($this->get_mobile_headers() as $mobile_header => $match_type) {
            if (isset($this->http_headers[$mobile_header])) {
                if (isset($match_type['matches']) && is_array($match_type['matches'])) {
                    foreach ($match_type['matches'] as $_match) {
                        if (str_contains($this->http_headers[$mobile_header], $_match)) {
                            return true;
                        }
                    }
                    return false;
    ...
    public function __call(string $name, array $arguments)
    {
        ...
        $rule_name = substr($name, 2);
        return $this->is($rule_name);
    }
    /**
    ...
    {
        if (!$this->has_user_agent()) {
            throw new Mobile_Detect_Exception('No valid user-agent has been set.', Mobile_Detect_Exception_Code::INVALID_USER_AGENT_ERR);
        }
        if ($this->is_user_agent_empty()) {
            return false;
        }
        // Cache check.
        try {
            $cache_key = $this->create_cache_key('mobile');
            $cache_item = $this->cache->get($cache_key);
            if ($cache_item !== null) {
                return $cache_item;
            }
            // Special case: Amazon CloudFront mobile viewer
            if ($this->get_user_agent() === self::$cloud_front_ua && $this->get_http_header('HTTP_CLOUDFRONT_IS_MOBILE_VIEWER') === 'true') {
                $this->cache->set($cache_key, true, $this->config['cacheTtl']);
                return true;
            }
            if ($this->has_http_headers() && $this->check_http_headers_for_mobile()) {
                $this->cache->set($cache_key, true, $this->config['cacheTtl']);
                return true;
            }
            $result = $this->match_user_agent_with_first_found_matching_rule();
            $this->cache->set($cache_key, $result, $this->config['cacheTtl']);
            return $result;
        } catch (Cache_Invalid_Argument_Exception|Cache_Exception|Psr_Invalid_Argument_Exception $e) {
            throw new Mobile_Detect_Exception("Cache problem in isMobile(): {$e->get_message()}", Mobile_Detect_Exception_Code::IS_MOBILE_ERR, $e);
        }
    }
    /**
    ...
    {
        if (!$this->has_user_agent()) {
            throw new Mobile_Detect_Exception('No user-agent has been set.', Mobile_Detect_Exception_Code::INVALID_USER_AGENT_ERR);
        }
        if ($this->is_user_agent_empty()) {
            return false;
        }
        // Cache check.
        try {
            $cache_key = $this->create_cache_key('tablet');
            $cache_item = $this->cache->get($cache_key);
            if ($cache_item !== null) {
                return $cache_item;
            }
            // Special case: Amazon CloudFront mobile viewer
            if ($this->get_user_agent() === self::$cloud_front_ua && $this->get_http_header('HTTP_CLOUDFRONT_IS_TABLET_VIEWER') === 'true') {
                $this->cache->set($cache_key, true, $this->config['cacheTtl']);
                return true;
            }
            foreach (static::$tablet_devices as $_regex) {
                $regex_string = $_regex;
                // "regex" is array of "strings"
                if (is_array($_regex)) {
                    $regex_string = implode('|', $_regex);
                }
                if ($this->match($regex_string, $this->get_user_agent())) {
                    $this->cache->set($cache_key, true, $this->config['cacheTtl']);
                    return true;
                }
                //                if (is_array($_regex)) {
            ...
            $this->cache->set($cache_key, false, $this->config['cacheTtl']);
            return false;
        } catch (Cache_Invalid_Argument_Exception|Cache_Exception|Psr_Invalid_Argument_Exception $e) {
            throw new Mobile_Detect_Exception("Cache problem in isTablet(): {$e->get_message()}", Mobile_Detect_Exception_Code::IS_TABLET_ERR, $e);
        }
    }
    /**
     ...
     * @throws MobileDetectException
     */
    public function is(string $rule_name): bool
    {
        if (!$this->has_user_agent()) {
            throw new Mobile_Detect_Exception('No user-agent has been set.', Mobile_Detect_Exception_Code::INVALID_USER_AGENT_ERR);
        }
        ...
        // Cache check.
        try {
            $cache_key = $this->create_cache_key($rule_name);
            $cache_item = $this->cache->get($cache_key);
            if ($cache_item !== null) {
                return $cache_item;
            }
            $result = $this->match_user_agent_with_rule($rule_name);
            // Cache save.
            $this->cache->set($cache_key, $result, $this->config['cacheTtl']);
            return $result;
        } catch (Cache_Invalid_Argument_Exception|Cache_Exception|Psr_Invalid_Argument_Exception $e) {
            throw new Mobile_Detect_Exception("Cache problem in is(): {$e->get_message()}", Mobile_Detect_Exception_Code::IS_MAGIC_ERR, $e);
        }
    }
    /**
     ...
     */
    public function match(string $regex, string $user_agent): bool
    {
        $match = (bool) preg_match(sprintf('#%s#is', $regex), $user_agent, $matches);
        // If positive match is found, store the results for debug.
        if ($match) {
            $this->matching_regex = $regex;
            $this->matches_array = $matches;
        }
        return $match;
    }
    /**
     ...
     */
    protected function match_user_agent_with_first_found_matching_rule(): bool
    {
        ...
        foreach ($this->get_rules() as $_regex) {
            ...
            if (is_array($_regex)) {
                foreach ($_regex as $regex_string) {
                    if ($this->match($regex_string, $this->get_user_agent())) {
                        return true;
                    }
                }
            } else if ($this->match($_regex, $this->get_user_agent())) {
                return true;
            }
        }
        return false;
     ...
     */
    protected function match_user_agent_with_rule(string $rule_name): bool
    {
        $result = false;
        ...
        if (false === empty($_rules[$rule_name])) {
            $regex_string = $_rules[$rule_name];
            if (is_array($_rules[$rule_name])) {
                $regex_string = implode('|', $_rules[$rule_name]);
            }
            $result = $this->match($regex_string, $this->get_user_agent());
     ...
     */
    public function prepare_version_no(string $ver): float
    {
        $ver = str_replace(['_', ' ', '/'], '.', $ver);
        $arr_ver = explode('.', $ver, 2);
        if (isset($arr_ver[1])) {
            $arr_ver[1] = @str_replace('.', '', $arr_ver[1]);
            // @todo: treat strings versions.
        }
        return (float) implode('.', $arr_ver);
    }
    /**
     ...
     */
    public function version(string $property_name, string $type = self::VERSION_TYPE_STRING): float|bool|string
    {
        if (empty($property_name) || !$this->has_user_agent()) {
            return false;
        }
        ...
        if (true === isset($properties[$property_name])) {
            // Prepare the pattern to be matched.
            // Make sure we always deal with an array (string is converted).
            $properties[$property_name] = (array) $properties[$property_name];
            foreach ($properties[$property_name] as $property_match_string) {
                $property_pattern = str_replace('[VER]', self::VERSION_REGEX, $property_match_string);
                // Identify and extract the version.
                preg_match(sprintf('#%s#is', $property_pattern), $this->user_agent, $match);
                if (false === empty($match[1])) {
                    return $type == self::VERSION_TYPE_FLOAT ? $this->prepare_version_no($match[1]) : $match[1];
                }
            }
        }
        return false;
    ...
    protected function create_cache_key(string $key): string
    {
        ...
        $http_headers_key = $this->has_http_headers() ? static::flatten_headers($this->http_headers) : '';
        $cache_key = "{$key}:{$user_agent_key}:{$http_headers_key}";
        $cache_key_fn = $this->config['cacheKeyFn'];
        if (!is_callable($cache_key_fn)) {
            throw new Cache_Exception('cacheKeyFn is not a function.');
        }
        return call_user_func($cache_key_fn, $cache_key);
    }
    public static function flatten_headers(array $http_headers): string
    {
        $key = '';
